...
ScripColorToFarm                    = "橙貨"  --Options: 橙貨/紫貨
...
Potion                              = "極錬精薬<hq>"     --what potion to use (false if none)
...
HubCities =
{
    ...
    },
    {
        ...
        aethernet = {
            aethernetZoneId = 131,
            aethernetName = "マーケット（サファイアアベニュー国際広場）",
...
function ProcessRetainers()
    ...
    elseif PathfindInProgress() or PathIsRunning() then
        return
    elseif not HasTarget() or GetTargetName() ~= "呼び鈴" then
        yield("/target 呼び鈴")
        return
    elseif not GetCharacterCondition(CharacterCondition.occupiedSummoningBell) then
        yield("/interact")
